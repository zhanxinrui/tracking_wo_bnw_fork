...
class Tracker:
    ...
    def step(self, blob):
        ...
        if self.public_detections:
            ...
            if dets.nelement() > 0:
                boxes, scores = self.obj_detect.predict_boxes(blob['img'], dets)  # 为什么直接给出数据中的det,而不自己通过rpn产生proposal，
            else:
        ...
        if det_pos.nelement() > 0:
            ...
            new_det_pos, new_det_scores, new_det_features = self.reid(blob, new_det_pos,
                                                                      new_det_scores)  # 通过reid生成新的keep
        ...
        self.inactive_tracks = [
            ...
            # inactive有一定的patient否则就移出Inactive
        ]
